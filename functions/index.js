...
exports.sendToDevice_test = functions.https.onCall(async (request) => {
  const tokens = request.data.tokens;
  const title = request.data.title;
  const body = request.data.body;
  const targetUserId = request.data.target_user_id.toString();
  ...
  // Generate an Id in the 'notificaitons' collection
  const documentID = firestore.collection("test_notifications").doc().id;
  ...
  try {
    ...
    // Save notification to firestore
    await firestore.collection("test_notifications").doc(documentID).set({
      source_user_id: sourceUserId,
      target_user_id: targetUserId,
      tokens: tokens,
      title: title,
